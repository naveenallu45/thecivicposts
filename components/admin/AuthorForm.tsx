...
export default function AuthorForm({ author }: AuthorFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">
          {author ? 'Password' : 'Password *'} {author && <span className="text-gray-500 text-xs">(leave blank to keep current)</span>}
        </label>
        <div className="relative">
          <input
            ...
            placeholder={author ? "Enter new password (minimum 6 characters) or leave blank" : "Enter password for author login"}
          />
          <button
            type="button"
